"use client";
import { FaHeart } from "react-icons/fa";

export default function TrendingMaterials() {
	const trendingMaterials = [
		{
			title: "CHM12 — Lab Report Template (UNN)",
			author: "@Student M",
			likes: "2.1K",
			price: "0.30 XLM",
		},
		{
			title: "MTH111 — Calculus Notes 2024",
			author: "@John Doe",
			likes: "1.8K",
			price: "0.50 XLM",
		},
		{
			title: "PHY101 — Mechanics Summary",
			author: "@Alice Smith",
			likes: "1.2K",
			price: "0.20 XLM",
		},
	];

	return (
		<div className="p-6 bg-white border border-gray-100 rounded-xl shadow-sm">
			<div className="flex justify-between items-center mb-6">
				<h3 className="font-bold text-gray-900">Trending Materials</h3>
				<button className="text-indigo-600 hover:text-indigo-700 font-medium text-sm">
					See All
				</button>
			</div>
			<div className="space-y-4">
				{trendingMaterials.map((item, i) => (
					<div
						key={i}
						className="flex gap-4 items-start group hover:bg-gray-50 p-2 -mx-2 rounded-lg transition-colors cursor-pointer"
					>
						<div className="w-16 h-20 bg-gradient-to-br from-indigo-100 to-blue-50 rounded-md flex flex-col items-center justify-center border border-indigo-100/50 flex-shrink-0">
							<span className="text-[10px] text-indigo-400 font-bold uppercase tracking-wider">Note</span>
						</div>
						<div className="flex-1 min-w-0 py-1">
							<h4 className="font-semibold text-sm text-gray-900 truncate mb-1">{item.title}</h4>
							<p className="text-xs text-gray-500 mb-2 truncate">{item.author}</p>
							<div className="flex justify-between items-center text-xs">
								<span className="flex items-center text-gray-500 gap-1">
                                    <FaHeart className="w-3 h-3 text-rose-400" />
                                    {item.likes}
                                </span>
								<span className="font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">
									{item.price}
								</span>
							</div>
						</div>
					</div>
				))}
			</div>
		</div>
	);
}
